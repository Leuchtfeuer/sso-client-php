...
if (!$version) {
	$data = 'user=' . $user . '&tpa_id=' . $tpa_id . '&expires=' . $expires;
} else {
	$data = 'version=' . $version . '&user=' . $user . '&tpa_id=' . $tpa_id . '&expires=' . $expires . '&action=' . $action . '&flags=' . $flags . '&userdata=' . $userdata;
}
if ($debugflag) {
		...
		switch ($action) {
			case 'logon':
				if ($flags["create_modify"] == "1") {
					...
					if ($debugflag) {
						print('<br>Executed function sso with params : ' . $user . ' ' . $_SERVER["REMOTE_ADDR"] . ' ' . $_SERVER["HTTP_USER_AGENT"] . ' ' . $url . ' ' . $version . ' create_modify ' . $userdata);
					}
					...
					if ($debugflag) {
						print('<br>Executed function sso again with params : ' . $user . ' ' . $_SERVER["REMOTE_ADDR"] . ' ' . $_SERVER["HTTP_USER_AGENT"] . ' ' . $url . ' ' . $version . ' logon ' . $userdata);
					}
				} else {
					$sso_values = sso($user, $_SERVER["REMOTE_ADDR"], $_SERVER["HTTP_USER_AGENT"], $url, $version, "logon", $userdata);
					if ($debugflag) {
						print('<br>Executed function sso with params : ' . $user . ' ' . $_SERVER["REMOTE_ADDR"] . ' ' . $_SERVER["HTTP_USER_AGENT"] . ' ' . $url . ' ' . $version . ' logon ' . $userdata);
					}
				}
				break;
...
function getvars() {
	...
	if ($debugflag) {
		...
		print('<br>Signature ssohex2bin and back: ' . bin2hex($sign));
	}
}

function ssohex2bin($data) {
	$len = strlen($data);
	$newdata = '';
	for ($i = 0; $i < $len; $i += 2) {
